...
function joinRoom( socket, room ) {
  var index, usersInRoom, userSocketId, usersInRoomSummary;
  ...
  socket.broadcast.to(room).emit('message', {
	...
	text: nickNames[ socket.id ] + ' さんが ' + room + ' に入室しました。'
  });
  ...
  if (usersInRoom.length > 1) {
	...
	usersInRoomSummary = '現在 ' + room + ' にいるユーザー : ';
    ...
    if (name.indexOf('Guest') == 0) {
      ...
      if (namesUsed.indexOf(name) == -1) {
        ...
        socket.broadcast.to(currentRoom[socket.id]).emit('message', {
          text: previousName + ' さんの名前が ' + name + ' に変わりました。'
          // text: previousName + ' is now known as ' + name + '.'
        });
      } else {
        socket.emit('nameResult', {
          success: false,
          message: 'その名前はもう使われています。'  // その名前はもう使われています。
